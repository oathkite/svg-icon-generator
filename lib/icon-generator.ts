...
import { type IconPattern, findMatchingPattern } from "./icon-patterns";
import { type IconSearchResult, getIconSVG, searchIcons } from "./icon-search";
...
class IconGenerationService {
	...
	private async tryIconifySearch(
		prompt: string,
	...
	private async tryPatternSearch(
		prompt: string,
	): Promise<IconGenerationResult | null> {
	...
	private async tryVertexAIGeneration(
		prompt: string,
	): Promise<IconGenerationResult | null> {
		try {
