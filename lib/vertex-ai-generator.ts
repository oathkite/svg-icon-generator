...
Vector-style illustration, high contrast.`;
	...
	async generateIconWithFallback(prompt: string): Promise<{
		...
		if (base64Image) {
			...
			return {
				svg:
					this.generateFallbackSVG(prompt) || this.createPlaceholderSVG(prompt),
				source: "vertex-ai",
				confidence: 85,
	...
	private generateFallbackSVG(prompt: string): string | null {
		...
		const fallbackPatterns: { [key: string]: string } = {
			メール:
				'<svg viewBox="0 0 24 24" width="24" height="24" xmlns="http://www.w3.org/2000/svg"><rect x="2" y="4" width="20" height="16" rx="2" stroke="currentColor" stroke-width="2" fill="none"/><path d="m22 7-10 5L2 7" stroke="currentColor" stroke-width="2" fill="none"/></svg>',
			ホーム:
				'<svg viewBox="0 0 24 24" width="24" height="24" xmlns="http://www.w3.org/2000/svg"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" stroke="currentColor" stroke-width="2" fill="none"/><polyline points="9,22 9,12 15,12 15,22" stroke="currentColor" stroke-width="2" fill="none"/></svg>',
